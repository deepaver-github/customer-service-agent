...
@Component({
  ...
  template: `
    <div class="flex h-full min-w-0 flex-1 flex-col bg-surface">
      <header class="flex flex-wrap items-center gap-3 border-b border-ink-200 bg-surface/85 px-4 py-3
                     sm:flex-nowrap sm:gap-3.5 sm:px-7 sm:py-3.5">
        <h1 class="flex-1 font-display text-[17px] font-semibold tracking-tight">Dashboard</h1>

        <form (submit)="onSearch($event)" class="relative order-last w-full sm:order-none sm:w-[260px] md:w-[320px]">
              ...
              <tbody>
                @for (p of recent(); track p.id) {
                  <tr class="cursor-pointer border-b border-ink-100 transition last:border-b-0 hover:bg-surface"
                    ...
                    <td data-label="NDIS #" class="px-5 py-3 font-mono text-[12.5px]">{{ p.ndis_number }}</td>
                    <td data-label="Disability" class="px-5 py-3 text-[13px] capitalize">
                      {{ (p.primary_disability_category || '—').replace('_', ' ') }}
                    </td>
                    <td data-label="Primary contact" class="px-5 py-3 text-[13px]">
